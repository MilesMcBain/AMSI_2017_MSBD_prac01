...
PRAC_HOME <- "/users/ugrad/amsi_705/MSBigData/"
```
...
```{r, message=FALSE}
tb_cases <- 
  read_csv(file.path(PRAC_HOME,"Practicals/prac01/data","tb.csv"))
...
```{r, message = FALSE}
#initialise spark
Sys.setenv(SPARK_HOME = file.path(PRAC_HOME,"spark-1.6.3-bin-hadoop2.6"))

library(SparkR, lib.loc = c(file.path(SPARK_HOME, "R", "lib")))
sc <- sparkR.init(master = "local[*]", 
                  sparkEnvir = list(spark.driver.memory="2g"))
sqlContext <- sparkRSQL.init(sc)

m4_data <- read.parquet(sqlContext, path = file.path(PRAC_HOME,"Practicals/prac01/data/m4_parquet"))
```
