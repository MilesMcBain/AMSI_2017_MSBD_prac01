...
m4_data <- spark_read_csv(sc, "M4_data", m4_data_file, overwrite = TRUE)
```
...
Standardising using `dplyr` and Spark:
```{r}
m4_data <-
  m4_data %>%
  group_by(route) %>%
  mutate(standard_transit = (transit - mean(transit)/sd(transit)))

head(m4_data)
```


Given our data, the objective is to transform it to present features that will offer traction to our chosen modelling. 
