...
# Prerequisites
* The file `prerequisites.R` contains the installation code for the packages needed to run this prac.
...
The motorway is broken up into a series of Eastbound and Westbound "loops" for which we have the recorded average transit time of vehicles calculated over consecutive 3 minute intervals from March to November in 2010.

### Initialisation
```{r, message = FALSE}
#initialise data file paths
m4_data_file <- here("data","M4_data.csv")
loop_lengths_file <- here("data", "RouteLengthApprox.csv")
...
#load data to spark instance
customSchema <- structType(
    structField("Date", "timestamp"),
    structField("route", "integer"),
    structField("transit", "integer"))

m4_data <- read.df(sqlContext, 
                    path = m4_data_file, 
...
**Discuss**: is this Tidy Data?

### Feature Engineering and Preprocessing using Dplyr/Spark

#### Preprocessing
An issue we have to consider is that not all routes (sections of road) are the same length. We'll standardise the transit times, but alternatively you could join the approximate route lengths from the csv file in `./data/`. And use that to model transit speed. This is left as an exercise.
...
#### Feature engineering
Given our data, the objective is to transform it to present features that will offer traction to our chosen modelling. A common example is processing of timestamps. Here `date`, a timestamp with granularity in seconds will probably not be useful in detecting the signal associated with transit time. Common sense would tell us that the hour of the day and day of the week may be more useful covariates.
